...
* :cloud: 支持本机、从机、七牛、阿里云 OSS、腾讯云 COS、又拍云、OneDrive (包括世纪互联版) 、S3（完美兼容 B2 Storage）作为存储端
* :outbox_tray: 上传/下载 支持客户端直传，支持下载限速，支持上传前创建占位文件
* 🌈 上传任务管理器支持多任务并行上传、任务排序、过滤、查看任务详情
* 💾 可对接 Aria2 ，实现批量离线下载，可使用多个从机机点分担下载任务
* 📚 在线 压缩/解压缩、多文件流式打包下载
* 💻 覆盖全部存储策略的 WebDAV 协议支持
* :zap: 拖拽上传、目录上传、流式或自定义分片大小上传处理
* :card_file_box: 文件拖拽管理、分页显示、全站搜索、批量获取直链
*  :family_woman_girl_boy:   多用户、用户组
* :link: 创建文件、目录的分享链接，可设定密码、自动过期、一键发送链接
